...
\begin{frame}[label=variance_scaling_main]{Variance and the Square Root of Time}
    ...
    For two years, the total change is the sum of two independent normal changes:
    \[
        \Delta X_2=\Delta X_{1,a}+\Delta X_{1,b}.
    \]

    \begin{block}{Key rule}
        Means add and variances add:
        \[
            \Delta X_2\sim\Normal(0,2),
            \qquad
            \operatorname{sd}(\Delta X_2)=\sqrt{2}.
        \]
    \end{block}
...
\begin{frame}{Volatility Scaling Example}
    If annual volatility is \(20\%\), then under square-root-time scaling:
    \[
        \sigma_{\text{1 month}}
        =
        20\%\sqrt{\frac{1}{12}}
    ...
    \begin{alertblock}{Common mistake}
        Do not divide annual volatility by 12 to get monthly volatility.
        Divide variance by 12, or divide standard deviation by \(\sqrt{12}\).
    \end{alertblock}
\end{frame}
...
\begin{frame}{Brownian Motion as a Limit}
    ...
    \begin{tikzpicture}[x=0.9cm,y=0.6cm]
        ...
        \draw[->] (0,-2.2) -- (0,2.5) node[above] {\(z_t\)};
        \draw[thick,blue]
            (0,0) -- (0.7,0.4) -- (1.2,-0.2) -- (1.8,0.6) -- (2.4,0.2)
            -- (3.0,1.0) -- (3.8,0.1) -- (4.3,0.7) -- (5.0,-0.4)
            -- (5.7,-0.1) -- (6.1,-1.0) -- (6.8,-0.2) -- (7.4,-0.8)
            -- (8.0,0.0) -- (8.7,0.5) -- (9.2,-0.3) -- (10,0.2);
        \node[blue] at (7.2,1.7) {jagged continuous path};
    \end{tikzpicture}
    \end{center}
...
\begin{frame}{Why Stock Prices Need Percentage Shocks}
    ...
    \[
        dS=a\,dt+b\,dz
    \]
    gives constant dollar drift and constant dollar volatility.

    \begin{alertblock}{Problem}
        A daily standard deviation of 2 dollars is huge for a 10 dollar stock and small for a 200 dollar stock.
    \end{alertblock}

    Stock returns are naturally proportional:
    \begin{itemize}
        \item expected compensation is quoted as percent return;
        \item volatility is quoted as percent volatility;
        \item compounding acts on returns, not dollar changes.
    \end{itemize}
\end{frame}
